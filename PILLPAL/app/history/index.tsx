...
export default function HistoryScreen() {
  ...
  const handleDownloadPDF = async () => {
    if (history.length === 0) {
      Alert.alert("No Data", "There is no history available to download.");
      return;
    }
    ...
    try {
      ...
      const groupedHistory = history.reduce((acc, dose) => {
        const date = new Date(dose.timestamp).toLocaleDateString();
        if (!acc[date]) {
          acc[date] = [];
        }
        acc[date].push(dose);
        ...
        <html>
          <head>
            <style>
              ...
              h1 { color: #1a8e2d; text-align: center; }
              .date-header { font-weight: bold; margin-top: 20px; color: #333; }
              .medication-card { 
      ...
      // Add history data to HTML
      Object.entries(groupedHistory).forEach(([date, doses]) => {
        ...
        doses.forEach(dose => {
          ...
          const statusClass = dose.taken ? 'taken' : 'missed';
          
          htmlContent += `
            <div class="medication-card">
              <div class="medication-name">${dose.medication?.name || 'Unknown Medication'}</div>
              <div>Dosage: ${dose.medication?.dosage || 'N/A'}</div>
              <div>Time: ${time}</div>
      ...
      const downloadsDir = FileSystem.documentDirectory + 'PillPal_History.pdf';
      await FileSystem.copyAsync({ from: uri, to: downloadsDir });

      // Share the PDF
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(downloadsDir, {
          mimeType: 'application/pdf',
          dialogTitle: 'Share Medication History',
          UTI: 'com.adobe.pdf'
        });
      } else {
        Alert.alert(
          "PDF Saved", 
          `Your medication history has been saved to: ${downloadsDir}`,
          [{ text: "OK" }]
        );
      }
    } catch (error) {
